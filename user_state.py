...
from lights_out import Bot_for_Sasha
...
class UserState():
	...
	def update_by_callback(cls, user_id, message_id, data: str):
		...
		if page_id == 'game':
			data = eval(data)
			state['info'] = Bot_for_Sasha.board_action(user_id, data['row'], data['col'])
			cls._update_game_view(user_id)
	...
	def _build_user_state(cls, user_id):
		state = {}
		state['info'] = info = Bot_for_Sasha.get_user_info(user_id)
